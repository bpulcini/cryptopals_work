...
with open(filename, 'r') as f:
    ...
    for line_num, line in enumerate(f):
        line_key_results = []
        for i in range(256):
            line_key_results.append([single_byte_xor(line.rstrip(), i), line_num, i])
    ...
    for x in all_line_results:
        print "Deciphered line " + str(x[1]) + " with key " + str(hex(x[2])) + " (ascii character " + chr(x[2]) + ") and got messsage: \"" + str(x[0]).rstrip() + "\" "
